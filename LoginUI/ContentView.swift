...
struct SignIn : View {
    ...
    var body : some View{
        ZStack{Color(#colorLiteral(red: 0.975846827, green: 0.8125091791, blue: 0.8340031505, alpha: 1))
...
func signUpWithEmail(email: String,password : String,compeletion: @escaping(Bool,String)->Void){
    Auth.auth().createUser(withEmail: email, password: password){
        (res, err) in
        
        if err != nil{
            
            compeletion(false,(err?.localizedDescription)!)
            return
        }
        compeletion(true,(res?.user.email)!)
    }
}
